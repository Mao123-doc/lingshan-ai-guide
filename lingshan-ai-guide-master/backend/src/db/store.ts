...
export function getDailyStats(days: number = 7): DailyStats[] {
  ...
  const sorted = Object.values(stats).sort((a, b) => b.date.localeCompare(a.date));
  return sorted.slice(0, days);
}

export function getHotQuestions(days: number = 7): Array<{ question: string; count: number }> {
  const recent = getDailyStats(days);
  const merged: Record<string, number> = {};
  for (const day of recent) {
    for (const q of day.hot_questions) {
      merged[q.question] = (merged[q.question] || 0) + q.count;
    }
  }
...
export function getEmotionDistribution(days: number = 7): Record<string, number> {
  ...
  for (const day of recent) {
    for (const [emotion, count] of Object.entries(day.emotions)) {
      dist[emotion] = (dist[emotion] || 0) + count;
    }
  }
  return dist;
}
