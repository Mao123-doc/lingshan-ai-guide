...
adminRouter.get('/reports/sentiment', (req: Request, res: Response) => {
  ...
  res.json({
    ...
    total_queries: count,
    avg_sentiment: Math.round((positive / total) * 100),
    ...
    summary: `本${days === 1 ? '日' : '周'}游客咨询${count}次，正面情感占比${Math.round((positive/total)*100)}%。${
      topSpots.length > 0 ? `最受关注景点：${topSpots[0]?.name || '灵山大佛'}。` : ''}系统运行稳定，知识库覆盖${getKnowledgeStats().chunkCount}个知识分块。`,
  });
});
