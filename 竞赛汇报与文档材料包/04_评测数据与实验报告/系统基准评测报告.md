...
该历史提交后的 5 题 Runtime Smoke（#1、#3、#31、#37、#42）确认 health `ok`、LLM 非 offline、
Vector service 可用，5/5 请求均为真实 Full-RAG。它不代表 2026-09-21 当前运行环境已经通过同一门禁。
...
- Scene Gold：40/40；Route Benchmark：60/60，hard violations 0。
- RAG smoke：向量服务恢复后 `vector_search=true`，但 LLM provider 调用失败，`full_rag_count=0`。
- 7-profile 消融完成 7×50 API 调用，但质量门禁为 0/7；新目录中的 90.0% Full Retrieval 等数值仅作诊断观测。
- 当前路线资产为 23 个 spot 记录、30 条 edge 记录；结构化知识 health 为 22 个 indexed spots。二者不得合并表述为“23 景点、31 道路已核验”。
